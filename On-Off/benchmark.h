...
const uint32_t FULL_KEY_LENGTH = 13 /*+ 8*/;
...
public:
	...
	BenchMark(const char* _PATH, const COUNT_TYPE _T) :
		...
		while (fread(&full_key, FULL_KEY_LENGTH, 1, file) > 0) {
			memcpy(&item, full_key, sizeof(DATA_TYPE));
			if (number % LENGTH == 0)
				T += 1;
			number += 1;
	...
	void SketchError(uint32_t section, uint32_t memory_in_byte) {
		AbsVector PEs = {
				new OO_PE<DATA_TYPE, COUNT_TYPE>(3, memory_in_byte / 3.0 / (BITSIZE + sizeof(COUNT_TYPE))),
				new MP_OO_PE<DATA_TYPE, COUNT_TYPE>(3, memory_in_byte / 3.0 / (BITSIZE * 3 + sizeof(PyramidCounter))),
		};
	...
	void TopKError(double alpha, uint32_t memory_in_byte) {
		AbsVector FPIs = {
				new OO_FPI<DATA_TYPE, COUNT_TYPE, 8>(memory_in_byte),
				new MP_OO_FPI<DATA_TYPE, COUNT_TYPE, 8>(memory_in_byte),
		};
