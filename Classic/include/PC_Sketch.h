...
public:
	...
	PC_Sketch(int _word_num, int _d, int word_size, int hash_seed);
...
//For PC_Sketch.h, the word_size must be 64;
PC_Sketch::PC_Sketch(int _word_num, int _d, int word_size, int hash_seed = 1000)
{
...
void PC_Sketch::Insert(const char* str)
{
	...
	hash_value = (bobhash[0]->run(str, KEY_LEN));
	word_index = (hash_value & ((1ll << word_index_size) - 1)) % word_num;
	...
	hash_value = (bobhash[1]->run(str, KEY_LEN));
	word_index = (hash_value & ((1ll << word_index_size) - 1)) % word_num;
	hash_value >>= word_index_size;
	...
	for (int i = 0; i < d; i++)
	{
		...
		int	g = (bobhash[i + d]->run(str, KEY_LEN)) % 2;
...
int PC_Sketch::Query(const char* str)
{
...
void PC_Sketch::Delete(const char* str)
{
...
void PC_Sketch::down_carry(int index)
{
	int left_or_right;
...
void PC_Sketch::carry(int index)
{
	int left_or_right;
...
int PC_Sketch::get_value(int index)
{
	int left_or_right;
